...
# 1. Load the data (no headers)
gpasat <- read.table("Data/gpasat.txt", header = FALSE)

# 2. Assign meaningful names
colnames(gpasat) <- c("GPA", "SAT")

# 3. Correlation in the full applicant pool
full_cor <- cor.test(gpasat$GPA, gpasat$SAT)
print(full_cor)

# 4. Determine who ends up at State College:
#    – admitted if SAT + 200*GPA > 1550
#    – but those with SAT + 200*GPA > 1850 go elsewhere
accepted     <- subset(gpasat, SAT + 200*GPA > 1550)
matriculated <- subset(accepted, SAT + 200*GPA <= 1850)

# 5. Correlation among matriculated students
