...
numerical_predictors <- c("age", "traveltime", "studytime", "failures",
                          "famrel", "freetime", "goout", "Dalc", "Walc",
                          "health", "absences")

# Subset the data for numerical predictors and the outcome
data_numeric <- data %>%
  select(all_of(c(numerical_predictors, outcome)))


head(data)

```
## Test out some simple models with and without log transforms
```{r}
# Create scatterplot of Portuguese language proficiency vs absences
# Create pairs plot using GGally
ggpairs(data_numeric,
        title = "Pairs Plot of Numerical Predictors and Outcome",
        lower = list(continuous = wrap("smooth", alpha = 0.3)),
        diag = list(continuous = "densityDiag"),
        upper = list(continuous = "cor"))


# Fit a simple linear regression model: G3port ~ absences
linear_model <- lm(G3por ~ ., data = data)
...
# Check for zero or negative values before log transformation
if(any(data$G3port <= 0) | any(data$absences <= 0)) {
  stop("G3port and absences must be positive for log transformation.")
}

# Apply logarithmic transformation to both response and predictor
data <- data %>%
  mutate(log_G3port = log(G3port),
         log_absences = log(absences))


# Create scatterplot of log-transformed variables
ggplot(data, aes(x = log_absences, y = log_G3port)) +
  geom_point(color = "purple") +
  labs(title = "Log-Log Scatterplot: Portuguese Language Proficiency vs Absences",
       x = "Log Number of Absences (log_absences)",
       y = "Log Portuguese Language Grade (log_G3port)") +
  theme_minimal()
