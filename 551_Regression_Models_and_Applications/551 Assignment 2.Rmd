...
# Set up parameters
p_yes <- 0.5          # Proportion of 'yes' responses (assumed to be 50%)
n_total <- 1000       # Total sample size for both options

# Option (a) - Simple Random Sample
n_latino_a <- 150     # Sample size for Latinos in option (a)
n_nonlatino_a <- 850  # Sample size for Non-Latinos in option (a)

# Option (b) - Oversampling Latinos
n_latino_b <- 300     # Sample size for Latinos in option (b)
n_nonlatino_b <- 700  # Sample size for Non-Latinos in option (b)
...
# Calculate the mean and standard deviation of ICER
mean_icer <- mean(icer_sim)
sd_icer <- sd(icer_sim)

# Standard error of ICER
se_icer <- sd_icer / sqrt(n_sim)

# 50% and 95% confidence intervals
z_50 <- 0.674  # z-value for 50% confidence
z_95 <- 1.96   # z-value for 95% confidence

# 50% confidence interval
ci_50_lower <- mean_icer - z_50 * se_icer
ci_50_upper <- mean_icer + z_50 * se_icer

# 95% confidence interval
ci_95_lower <- mean_icer - z_95 * se_icer
ci_95_upper <- mean_icer + z_95 * se_icer

# Print results
cat("Estimated mean ICER:", mean_icer, "\n")
...
mean_effect_diff <- 3.0
se_effect_diff_2 <- 2.0  # New standard error for effectiveness
df_effect <- 100  # degrees of freedom for effectiveness difference
...
# Simulate effectiveness differences with new SE from a t-distribution
effect_diff_sim_2 <- mean_effect_diff + rt(n_sim, df = df_effect) * se_effect_diff_2

# Calculate the ICER with the new effectiveness differences
icer_sim_2 <- cost_diff_sim / effect_diff_sim_2

# Calculate the mean and standard deviation of the new ICER
mean_icer_2 <- mean(icer_sim_2)
sd_icer_2 <- sd(icer_sim_2)

# Standard error of new ICER
se_icer_2 <- sd_icer_2 / sqrt(n_sim)

# z-values for 50% and 95% confidence intervals
z_50 <- 0.674
z_95 <- 1.96

# 50% confidence interval for the new ICER
ci_50_lower_2 <- mean_icer_2 - z_50 * se_icer_2
ci_50_upper_2 <- mean_icer_2 + z_50 * se_icer_2

# 95% confidence interval for the new ICER
ci_95_lower_2 <- mean_icer_2 - z_95 * se_icer_2
ci_95_upper_2 <- mean_icer_2 + z_95 * se_icer_2

# Print results for the new scenario
cat("With SE = 2.0\n")
cat("Estimated mean ICER:", mean_icer_2, "\n")
cat("50% confidence interval for ICER:", ci_50_lower_2, "to", ci_50_upper_2, "\n")
cat("95% confidence interval for ICER:", ci_95_lower_2, "to", ci_95_upper_2, "\n")

# Remove extreme ICER values for plotting
icer_filtered_2 <- icer_sim_2[effect_diff_sim_2 > 0 & abs(icer_sim_2) < 1000]

# (1) Scatterplot of cost differences vs. effectiveness differences with SE = 2.0
plot(cost_diff_sim, effect_diff_sim_2,
     xlab = "Cost Difference",
     ylab = "Effectiveness Difference",
...
for (n in n_values) {
  # Simulate data using the function from Exercise 6.2
...
# Plot the standard error of the intercept as a function of n
...
# Plot the standard error of the slope as a function of n
...
# (c) Add the formula of the fitted line to the graph
